...
export default function ZoneManagerDashboard() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      // Pour un zone manager, on ne compte que les stocks de son agence
      const agencyStocks = userData.agencies_id
        ? stocks.filter((stock) => stock.agencies_id === userData.agencies_id)
        : stocks;

      // Calculer le total de stock par produit (seulement pour son agence)
        ...
        <section className="mb-6">
          <div className="rounded-[28px] border-2 border-red-200 bg-red-50/80 p-6 shadow-[0_18px_50px_rgba(239,68,68,0.15)] backdrop-blur">
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                onClick={() => router.push("/zone_manager/stocks?alert=true")}
