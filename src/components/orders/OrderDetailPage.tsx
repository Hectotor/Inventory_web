...
export function OrderDetailPage({ orderId, backUrl, backLabel = "Retour" }: OrderDetailPageProps) {
  ...
  const loadOrder = async (orderId: string, companyId: string) => {
    try {
      // Charger la commande
      ...
      // Vérifier que la commande appartient à la même company
      if (orderData.company_id !== companyId) {
        setError("Accès non autorisé");
        return;
      }

      // Charger les informations du client
      const customerDoc = await getDoc(doc(db, "users", orderData.customer_id));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <section className="rounded-[32px] border border-white/60 bg-white/85 p-6 shadow-[0_24px_70px_rgba(15,23,42,0.1)] backdrop-blur">
          <h2 className="text-lg font-semibold mb-6">Informations de la commande</h2>
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-base font-semibold text-[#111827]">
                #{order.orders_id.slice(0, 8).toUpperCase()}
              </p>
            </div>
            ...
            <div className="pt-4 border-t border-zinc-200">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-[#6B7280]">Total HT:</span>
                <span className="text-base font-semibold text-[#111827]">
                  {order.total_ht.toFixed(2)} € HT
                </span>
              </div>
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-[#6B7280]">Total TVA:</span>
                <span className="text-sm text-[#6B7280]">
                  {(Math.round((order.total_ttc - order.total_ht) * 100) / 100).toFixed(2)} €
                </span>
              </div>
              <div className="flex items-center justify-between pt-2 border-t border-zinc-200">
                <span className="text-base font-semibold text-[#111827]">Total TTC:</span>
          ...
          {customer ? (
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-[#111827]">{customer.email}</p>
              </div>
              ...
              {customer.phone && (
                <div>
                  ...
                  <p className="text-sm text-[#111827]">{customer.phone}</p>
                </div>
              )}
            ...
            <div className="text-center py-8 text-[#6B7280]">
              <p className="text-sm">Informations client non disponibles</p>
            </div>
          )}
        </section>
      </div>
