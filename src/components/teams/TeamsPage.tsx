...
export function TeamsPage({ filterByAgency = false }: TeamsPageProps) {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      const userData = userSnapshot.data() as { company_id?: string; role?: string; agencies_id?: string };
  ...
  const filteredMembers = allTeamMembers.filter((member) => {
    // Filtrer par agence si filterByAgency est true
    if (filterByAgency && userAgencyId && member.agencies_id !== userAgencyId) {
      return false;
    }
    ...
    if (searchQuery) {
      ...
      const companyName = member.company_name?.toLowerCase() || "";
      if (
  ...
  }, [filterRole, filterAgency, filterStatus, searchQuery, allTeamMembers, filterByAgency, userAgencyId]);
    ...
    try {
      ...
      if (editingMember) {
        ...
        if (memberData.role === "customer") {
          ...
          updateData.city = memberData.city || null;
          updateData.country = memberData.country || null;
        } else {
          // Supprimer les champs spécifiques aux clients si le rôle change
        ...
        if (memberData.role === "customer") {
          ...
          if (memberData.city) {
            createData.city = memberData.city;
          }
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-4">
            <span className="text-sm text-[#6B7280]">
              {filteredMembers.length} / {allTeamMembers.length}{" "}
              {allTeamMembers.length === 1 ? "membre" : "membres"}
            </span>
            <button
              type="button"
              onClick={() => {
