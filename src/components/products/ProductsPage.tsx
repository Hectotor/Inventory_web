...
export function ProductsPage({ canManageProducts }: ProductsPageProps) {
  ...
  // Nettoyer les aperçus d'images lors du démontage
  useEffect(() => {
    return () => {
  ...
  // Fonction pour supprimer une image du Storage à partir de son URL
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Si on modifie un produit, identifier les images qui seront supprimées
      let imagesToDelete: string[] = [];
      if (editingProduct) {
        const oldImages = editingProduct.image_urls || (editingProduct.image_url ? [editingProduct.image_url] : []);
        // Les images qui ne sont plus dans existingImageUrls doivent être supprimées
        imagesToDelete = oldImages.filter((oldUrl) => !existingImageUrls.includes(oldUrl));
      }

      // Combiner les URLs existantes et les nouvelles images
      ...
      if (selectedImageFiles.length > 0) {
        try {
          const uploadPromises = selectedImageFiles.map(async (file) => {
            const imageRef = ref(
              storage,
              `companies/${companyId}/products/${Date.now()}_${Math.random().toString(36).substring(7)}_${file.name}`,
            );
          ...
          imageUrls = [...imageUrls, ...newImageUrls].slice(0, 3); // Limiter à 3 images max
        } catch (error) {
      ...
      // Supprimer les anciennes images qui ont été retirées
  ...
  const handleEdit = (product: Product) => {
    ...
    setImagePreviews([]);
    // Charger les images existantes (support ancien format image_url et nouveau format image_urls)
  ...
  const handleDelete = async (productId: string) => {
    ...
    try {
      ...
      if (productDoc.exists()) {
        ...
        const images = productData.image_urls || (productData.image_url ? [productData.image_url] : []);
        
        // Supprimer toutes les images du Storage
        if (images.length > 0) {
          await Promise.all(images.map((url) => deleteImageFromStorage(url)));
        }
      }

      // Supprimer le document du produit
  ...
  const handleImageSelect = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    // Limiter à 3 images au total (existantes + nouvelles)
    const totalImages = existingImageUrls.length + selectedImageFiles.length + files.length;
    if (totalImages > 3) {
      alert("Vous ne pouvez ajouter que 3 images maximum par produit");
      return;
    }

    // Ajouter les nouveaux fichiers
    const newFiles = [...selectedImageFiles, ...files].slice(0, 3 - existingImageUrls.length);
    setSelectedImageFiles(newFiles);

    // Créer des aperçus locaux pour les nouvelles images
    const newPreviews = files.map((file) => URL.createObjectURL(file));
    setImagePreviews((prev) => [...prev, ...newPreviews].slice(0, 3 - existingImageUrls.length));
  ...
  const removeImage = (index: number, isExisting: boolean) => {
    if (isExisting) {
      ...
      setSelectedImageFiles((prev) => prev.filter((_, i) => i !== previewIndex));
    }
  };

  const resetForm = () => {
    // Nettoyer les aperçus d'images
    imagePreviews.forEach((preview) => {
      URL.revokeObjectURL(preview);
    });
    setFormData({
        ...
        <section className="rounded-[32px] border border-white/60 bg-white/85 p-6 shadow-[0_24px_70px_rgba(15,23,42,0.1)] backdrop-blur">
          ...
          <form onSubmit={handleSubmit} className="grid gap-4">
            ...
            <div className="grid gap-4">
              <div>
                <label className="text-xs uppercase tracking-[0.2em] text-[#6B7280] mb-2 block">
                  Images du produit (max 3)
                </label>
                <p className="text-xs text-[#6B7280] mb-3">
                  PNG ou JPG · 5MB max recommandé par image
                </p>
                <div className="grid grid-cols-3 gap-3">
                  {/* Afficher les images existantes */}
                  {existingImageUrls.map((url, index) => (
                    <div key={`existing-${index}`} className="relative group">
                      <div className="h-32 w-full overflow-hidden rounded-xl border border-zinc-200 bg-white">
                        <img
                          src={url}
                      ...
                      <button
                        ...
                        className="absolute top-1 right-1 h-6 w-6 rounded-full bg-red-500 text-white flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600"
                  ...
                  {imagePreviews.map((preview, index) => (
                    <div key={`preview-${index}`} className="relative group">
                      ...
                      <button
                        type="button"
                        onClick={() => removeImage(existingImageUrls.length + index, false)}
                        className="absolute top-1 right-1 h-6 w-6 rounded-full bg-red-500 text-white flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600"
                        title="Supprimer cette image"
                      >
                        ✕
                      ...
                      <div className="absolute bottom-1 left-1 right-1">
                        <p className="text-xs text-white bg-black/50 px-2 py-0.5 rounded truncate">
                          {selectedImageFiles[index]?.name}
                        </p>
                      </div>
                    </div>
                  ))}
                  {/* Slot vide pour ajouter une nouvelle image */}
                  {existingImageUrls.length + imagePreviews.length < 3 && (
                    <label
                      htmlFor="productImage"
                      className="h-32 w-full rounded-xl border-2 border-dashed border-zinc-300 bg-zinc-50 flex flex-col items-center justify-center cursor-pointer transition hover:border-zinc-400 hover:bg-zinc-100"
                    >
                ...
                <input
                  ...
                  className="hidden"
                  disabled={isLoading || isSaving || existingImageUrls.length + imagePreviews.length >= 3}
                />
                {existingImageUrls.length + imagePreviews.length >= 3 && (
